...
const BookRequests = () => {
  ...
  const fetchRequestsData = async () => {
    const res = await actions.fetchRequestData(authUserId);
    if (!res.error) {
      setReqs(res.buyRequestsRes);
      setAllReqs(res.allBuyRequestsRes);
      setAllSwapReqs(res.allSwapRequestsRes);
    } else {
  ...
  const handleCancel = async (reqId) => {
    const res = await actions.handleCancelBuyRQ(reqId);
    if (res.error) {
      toast.error(res.msg);
    } else {
  ...
  const deleteTheBook = async (reqId, type) => {
    if (type === 1) {
      const res = await actions.deleteBuyBooksData(reqId);
      if (res.error) {
        toast.error(res.msg);
      }
    } else {
      const res = await actions.deleteSwapBooksData(reqId);
      if (res.error) {
        toast.error(res.msg);
      }
    }
  ...
  const handleConfirm = async (req) => {
    const res = await actions.handleConfirmBuyRQ(req);
    if (!res.error) {
      allReqs.forEach((x) => {
  ...
  return (
    <div className="py-5 px-10">
      <div className="pb-5">
        <p className="text-3xl font-semibold">Buy requests to you</p>
      </div>
                ...
                <tr key={req.id}>
                  ...
                  <td className="whitespace-nowrap px-4 py-2 text-gray-700 text-center">
                    <div className="flex justify-center items-center">
                      <img
                        src={`${process.env.NEXT_PUBLIC_BASEURL}${req.book.images[0].image}`}
                        alt=""
