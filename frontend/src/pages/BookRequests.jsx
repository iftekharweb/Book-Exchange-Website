...
const BookRequests = () => {
  ...
  const handleCancel = async (reqId) => {
    try {
      const res = await axios.delete(`${import.meta.env.VITE_BASEURL}/buybooks/${reqId}/`);
      fetchRequests();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleConfirm = async (reqId, bookId) => {
    //
  }
  ...
  return (
    <div className="py-5 px-10">
      <div className="pb-5">
        <p className="text-3xl font-serif font-semibold">Books requests to you</p>
      </div>
              ...
              {currentReqs.map((req) => (
                <tr key={req.id}>
                  ...
                  <td className="whitespace-nowrap px-4 py-2 text-gray-700 text-center">
                    <div className="flex justify-center items-center">
                      <div className="flex justify-center items-center">
                        <button className="mr-3 border text-green-500 border-green-500 hover:bg-green-500 hover:text-white rounded-md font-semibold px-3 py-1">
                          Confirm
                        </button>
                        <button className="mr-3 border text-red-500 border-red-500 hover:bg-red-500 hover:text-white rounded-md font-semibold px-3 py-1" onClick={() => handleCancel(req.id)}>
                          Cancel
                        </button>
                      </div>
                    </div>
