...
const SelectBookToSwap = () => {
  const { authUserId, userToSwap, setUserToSwap, bookToSwap, setBookToSwap } = useStateContext();
  ...
  const handleCancel = () => navigate("/swap-requests");

  const fetchBooks = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_BASEURL}/user-books/?user=${userToSwap.id}`
      );
      if (res.data) {
        setBooks(res.data);
  ...
  return (
    <div className="p-0">
        {details && <ConfirmSwapModal handleDetails={handleDetails} book={currBook}/>}
      <section>
